#> pretty_crafter:find/shaped/check_each
### Copyright © 2022 赤石愛
...
## カテゴリ一致ならアイテムを比較
execute if data storage pretty_crafter: {mismatch:false} run function pretty_crafter:find/shaped/compare_items
## 一致レシピが未発見で、まだレシピが残っていたら繰り返し
data remove storage pretty_crafter: recipe_candidates[-1]
execute unless data storage pretty_crafter: found if data storage pretty_crafter: recipe_candidates[0] run function pretty_crafter:find/shaped/check_each
