...
## カテゴリが一致しているならアイテムも確認
execute if data storage pretty_crafter: {mismatch:false} run function pretty_crafter:remove/shapeless/compare_items
## 一致していなければ残すレシピとして保存
...
execute unless data storage pretty_crafter: found if data storage pretty_crafter: recipe_candidates[0] run function pretty_crafter:remove/shapeless/check_each
